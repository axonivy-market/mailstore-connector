...
Unlock the potential of Axon Ivy's Mailstore utility to streamline your process automation endeavors, simplifying email management within your business processes. This versatile utility:
...
```java
MessageIterator it = MailsStoreService.messageIterator("etherealImaps", "INBOX", "archive", true, MailStoreService.subjectMatches(".*Request [0-9]+.*"), new MessageComparator())
```
...
If you want to see utility logs, enable the `debug` switch.  
If your utility requires special settings, you can define them in the `properties` section.
...
Variables:
  mailstoreUtils:
    etherealImaps:
      ...
      protocol: 'imaps'
      # Host for store utility
      host: 'imap.ethereal.email'
      # Port for store utility (only needed if not default)
      port: -1
      # User name for store utility
      user: 'myname@ethereal.email'
      # Password for store utility
      # [password]
      password: '${encrypt:mypassword}'
      # show debug output for utility
      debug: true
      # Additional properties for store utility,
...
```yaml
      ...
      # com.axonivy.utils.oauth.BasicUserPasswordProvider for Basic Authentication
      # com.axonivy.utils.oauth.AzureOauth2UserPasswordProvider for AzureOauth2UserPasswordProvider
      userPasswordProvider: 'com.axonivy.utils.oauth.AzureOauth2UserPasswordProvider'
...
Variables:
  mailstoreUtils:
    localhostImapAzureOauth2Authentication:
      ...
      grantType: ''
  # login url microsoft azure
  azureOAuth:
    loginUrl: 'login.microsoftonline.com'
```
> [!NOTE]
> The variable path `mailstore-utils` is renamed to `mailstoreUtils` from 13.

4. Set Up the Authentication Provider
Before calling the mailstore utility, you need to provide an authentication provider.
```java
  Class<?> clazz = Class.forName("com.axonivy.utils.oauth.AzureOauth2UserPasswordProvider");
